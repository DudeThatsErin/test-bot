    ...
    async execute(interaction, client) {
        ...
        if (command.ownerOnly === 'yes') {
            if (!interaction.user.id === config.developer.id) {
                return interaction.reply({ content: `This is only a command Erin (DudeThatsErin#8061) can use. If you are seeing this in error use the \`${prefix}report\` command.`, ephemeral: true });
            }
        }
        ...
        if (command.modOnly === 'yes') {
            for (const ID of modRoles) {
                ...
                if (value == modRoles.length) {
                    interaction.reply({ content: `This is a command only moderators can use. You do not have the required permissions. Moderators have the <@&${modRoles[0]}> role or <@&${modRoles[1]}> role or <@&${modRoles[2]}> role  or <@&${modRoles[3]}> role. Please run \`${config.bot.prefix}report [issue]\` if you are seeing this in error.`, ephemeral: true });
                    return;
                }
            }
        }
        ...
        if (command.challengeMods === 'yes') {
            for (const ID of chllMod) {
                ...
                if (value == chllMod.length) {
                    interaction.reply({ content: `This is a command only challenge moderators can use. You do not have the required permissions. Challenge moderators have the <@&${chllMod[1]}> role. Please run \`${config.bot.prefix}report [issue]\` if you are seeing this in error.`, ephemeral: true });
                    return;
                }
            }
        }
        ...
        try {
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .addFields({
                    ...
                    name: '**Ways to Report:**',
                    value: `Run the \`${config.bot.prefix}report\` command, [Fill out this form](https://codinghelp.site/contact-us/) (Erin owns CodingHelp so that form goes directly to her), Message her on Discord, or Email her at me@dudethatserin.site\n\nPlease include all of the information in this embed (message) as well as any additional information you can think to provide. Screenshots are also VERY helpful. Thank you!`
                })
                .setTimestamp()
