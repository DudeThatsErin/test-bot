...
module.exports = {
    ...
    async execute(message, client) {
        ...
        const isBot = prefixRegex.test(message);
        if (message.content.startsWith(config.bot.prefix) || isBot) {
            ...
            if (command.botPerms.length > 0) {
                ...
                if (!clientChannelPermissions.has(command.botPerms)) {
                    ...
                    message.react('❌');
                    message.reply(`${config.bot.name} does not have the required permissions. If your DMs are open, I have sent you a DM on the matter. Just in case you don\'t have your DMs open, the permissions ${config.bot.name} needs are:\n\`\`\`${missingPermissions}\`\`\` and her current permissions are:\n\`\`\`${currentPermissions}\`\`\`If you feel you are receiving this message in error, open your DMs and run the command again to get ways to report this to the developer.`)
                    return;
                }
            }
